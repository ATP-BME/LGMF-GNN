...
def process_dynamic_fc(timeseries, window_size, window_stride, dynamic_length=None, sampling_init=None, self_loop=True):
    # Size of timeseries: batch x time x node
    # Size of output: batch x time x node x node
    ...
    for i in sampling_points:
        ...
        for _t in timeseries:
            fc = corrcoef(_t[i:i+window_size].T) # Calculate PC
...
def corrcoef(x):
    # Size of x: node x time
    # Size of output: node x node
