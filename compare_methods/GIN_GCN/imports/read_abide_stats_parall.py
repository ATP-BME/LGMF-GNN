...
def read_data(data_dir,onlyfiles=None):
...
def read_sigle_data(data_dir,filename,use_gdc=True): # use_gdc default false
    ...
    adj = A.tocoo()
    edge_att = np.zeros(len(adj.row)) # [40000]
    for i in range(len(adj.row)):
        edge_att[i] = pcorr[adj.row[i], adj.col[i]]

    edge_index = np.stack([adj.row, adj.col])
    edge_index, edge_att = remove_self_loops(torch.from_numpy(edge_index), torch.from_numpy(edge_att)) # array([array,array]) edge_attr [39800]
    edge_index = edge_index.long()
    edge_index, edge_att = coalesce(edge_index, edge_att, num_nodes,
                                    num_nodes)
    att = temp['corr'][()] # (200,200)
    label = temp['label'][()]

    att_torch = torch.from_numpy(att).float()
    y_torch = torch.from_numpy(np.array(label)).long()  # classification

    data = Data(x=att_torch, edge_index=edge_index.long(), y=y_torch, edge_attr=edge_att) # Data(x=[200,200],edge_index=[2,39800],edge_attr=[39800],y=[1])
