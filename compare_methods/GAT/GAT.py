...
class GAT(torch.nn.Module):
    ...
    def __init__(self, num_of_layers, num_heads_per_layer, num_features_per_layer, add_skip_connection=True, bias=True,
                 dropout=0.6, layer_type=LayerType.IMP3, log_attention_weights=False,logits_dim=1):
        super().__init__()
        ...
        self.graph_pred_layer = torch.nn.Linear(num_features_per_layer[-1],logits_dim)
    ...
    def forward(self, data):
        in_nodes_features, edge_index, edge_attr, batch = data.x, data.edge_index, data.edge_attr, data.batch
        tuple_data = (in_nodes_features,edge_index)
        node_emb = self.gat_net(tuple_data)
        graph_emb = self.graph_pool(node_emb[0],data.batch) # [12800bs*node_per_grah, 4emb_dim] [bs*node_per_grah]
        out =  self.graph_pred_layer(graph_emb)
        return out
...
class GATLayerImp3(GATLayer):
    ...
    def forward(self, data):
        ...
        return (out_nodes_features, edge_index) # N,NH,FOUT
