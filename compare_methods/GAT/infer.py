...
import time
import numpy as np
from main_pyg import get_training_args
...
def main():
    ...
    parser.add_argument('--node_dim', type=int, default=116,
    ...
    # parser.add_argument('--model_path', type=str, default='./model_corr_T1', 
    parser.add_argument('--model_path', type=str, default='./compare_openneuro/AAL', 
                        help='path to save checkpoint')
    parser.add_argument('--log_path', type=str, default='./log_compare_openneuro', 
    ...
    # args.model_path = os.path.join(args.model_path,str(args.fold))
    # if not os.path.exists(args.model_path):
    #     os.makedirs(args.model_path)

    ### automatic dataloading and splitting
    # dataset = PygGraphPropPredDataset(name = args.dataset)
    # get GAT model config
    config = get_training_args(args)
    ...
    if 'SRPBS' in args.dataroot:
        ...
        dataset_HBN = ABIDEDataset(args.dataroot,name,raw_folder='raw_aal') # openneuro raw_aal anding raw
    ...
    evaluator_acc = Evaluator(num_tasks=args.nclass,eval_metric='acc',fold=args.fold,model_name='gat')
    evaluator_auc = Evaluator(num_tasks=args.nclass,eval_metric='rocauc',fold=args.fold,model_name='gat')
    evaluator_prf = Evaluator(num_tasks=args.nclass,eval_metric='F1',fold=args.fold,model_name='gat')
    ...
    # build GAT model
    model = GAT(num_of_layers=config['num_of_layers'],
    ...
    # model.load_state_dict(torch.load('/home/sjtu/liushuyu/project/GIN/model_corr/gdc_0/0_1000.pth'))
    # model.load_state_dict(torch.load('/home/sjtu/liushuyu/project/GAT/model_corr/gdc_0/0_200.pth'))
    model.load_state_dict(torch.load(os.path.join(args.model_path,'gdc_'+str(args.fold),str(args.fold)+'_best.pth')))
