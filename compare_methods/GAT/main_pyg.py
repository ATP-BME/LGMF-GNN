...
def get_training_args(args):
    ...
    gat_config = {
        ...
        "num_heads_per_layer": [8, 4, 1],
        "num_features_per_layer": [args.node_dim, 64, 32, 16], #args.nclass],
        "add_skip_connection": False,  # hurts perf on Cora
        "bias": True,  # result is not so sensitive to bias
        "dropout": 0.6,  # result is sensitive to dropout
        "layer_type": LayerType.IMP3  # fastest implementation enabled by default impimentation3
    }
...
def main():
    ...
    # parser.add_argument('--dataroot', type=str, default='/data0/lsy/REST_meta_MDD/ROISignals_FunImgARCWF/', # REST
    ...
    args.model_path = os.path.join(args.model_path,'gdc_'+str(args.fold))
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if valid_perf[args.eval_metric] >= best_val_acc and epoch > 20:
    ...
    if 'classification' in dataset.task_type:
        best_val_epoch = np.argmax(np.array(valid_curve)[20:])+20
        best_train = max(train_curve)
    else:
        best_val_epoch = np.argmin(np.array(valid_curve)[20:])+20
