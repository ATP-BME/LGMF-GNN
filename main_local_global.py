...
if __name__ == '__main__':
    ...
    if 'MDD' in config['data']['dataset']: 
        balanced_names = np.genfromtxt("D:\data\depression\REST_meta_MDD\group\subject_IDs_selected.txt", dtype=str)
    if 'Anding' in config['data']['dataset']:
        balanced_names = np.genfromtxt("D:\data\depression\Anding\subject_IDs.txt", dtype=str)
    ...
    for fold in range(5,n_folds):
        ...
        test_ind = val_ind
        np.save('ind.npy',{'train_ind':train_ind,'val_ind':val_ind})
        ### cv val
